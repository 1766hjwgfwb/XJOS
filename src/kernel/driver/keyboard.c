...
static bool scrlook_state;  // scroll lock
static bool numlock_state;
static bool extcode_state;  // extended code
...
static void keyboard_wait() {
    ...
    } while (state & 0x02); // read until keyboard data is empty
}
...
static void set_leds() {
    u8 leds = (capslock_state << 2) | (numlock_state << 1) | (scrlook_state << 0);
    keyboard_wait();
...
void keyboard_handler(int vector) {
    ...
    u8 ext = 2;     // normal scancode, no shift
